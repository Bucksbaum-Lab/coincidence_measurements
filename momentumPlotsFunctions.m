...
function[] = particleKERhist(particle, partEnergyOut, mass, nbins, KEmax)
    hist(partEnergyOut(:,particle), nbins)
    xlim([0, KEmax])
    title(['KER(M/Z=' num2str(mass(particle)) '), shutter open'])
    xlabel('KER')
    ylabel('counts')
end
...
function[Xp, Yp, Noutp] = momentum2dDistPolar ...
            (parallel_proj, perpendicular_proj, rdata, adata, plotname, name1, name2, name3)

[Xp, Yp, Noutp] = hist3polar (abs(real(parallel_proj)), real(perpendicular_proj), rdata, adata);
